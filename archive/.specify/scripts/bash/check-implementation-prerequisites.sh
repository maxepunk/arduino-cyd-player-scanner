...
# Arduino-specific checks (only if .ino files exist)
if compgen -G "*.ino" > /dev/null 2>&1 || compgen -G "**/*.ino" > /dev/null 2>&1; then
  if ! command -v arduino-cli &> /dev/null; then
    echo "WARNING: Arduino CLI not found. Install with: curl -fsSL https://raw.githubusercontent.com/arduino/arduino-cli/master/install.sh | sh"
  else
    if ! arduino-cli core list | grep -q "esp32:esp32"; then
      echo "WARNING: ESP32 board package not installed. Install with: arduino-cli core install esp32:esp32"
    fi
  fi
fi
